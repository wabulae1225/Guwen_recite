...
/* ------------------------------------------------------------------
   ...
   拼音按字分开写（qióng lú），看词写拼音时字和音才对得上。
   题型：`写` 看拼音写词语（字生僻）、`读` 看词写拼音（有字变了读音）、
         `都考` 两种都出。
   词源：`课本`（课本自己注的音，最权威）、`成语`、`词典`。
   释义来源：`课本`（课本注释）、`成语`、`新华`、`自拟`，可靠性依次递减，
         页面照实标出来。四级都没有的词不存在——自拟那级兜底。
   例句：把被考的词用大括号圈起来，和 notes 板块一个写法。
         优先课本古诗文原句，其次成语的古典书证，再次其余课文。
